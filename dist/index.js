...
async function fetchFeed(tokens) {
    ...
    if (res.status === 401) {
        ...
        if (!refreshed)
            throw new Error("Session expired. Run `npx github:justpostedjobs/jpj-channel` to re-pair.");
...
function buildLaunchAgentPlist(workingDir) {
...
export function installLaunchAgent(workingDir) {
...
export function runReset() {
    ...
    try {
        if (fs.existsSync(settingsPath)) {
            ...
            const allow = settings?.permissions?.allow ?? [];
            const filtered = allow.filter((t) => !t.startsWith("mcp__jpj__"));
